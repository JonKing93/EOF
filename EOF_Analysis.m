...
%
%   MCnSig: The number of significant leading EOF modes after each Monte
%       Carlo iteration.
%
...
[matrix, pcaArgs, sigTest, ruleNArgs, p, convergeTest, nRotate, rotType, plotting, varNames] = setup( Data, varargin{:} );
...
if sigTest
    ...
    [s.sigExpVar, s.true_p] = mcthreshold(s.randExpVar, p); %, s.nSig] = eofSigThreshold(s.randExpVar, p, s.expVar);
    
    % Get the number of significant modes
    s.nSig =  find( s.expVar<=s.sigExpVar, 1, 'first'  )-1;
    
    % Record Monte Carlo convergence data.
    if convergeTest
        [s.MCsigExpVar, s.MCtrue_p, s.MCnSig] = ruleNConvergence( s.randExpVar, s.p, s.expVar );        
    end
end
...
% Do an error check on p NOW so that an error is not thrown AFTER the Monte
% Carlo process
if ~isscalar(p) || p<=0 || p>=1
    error('''p'' must be a scalar on the open interval (0,1)');
end
